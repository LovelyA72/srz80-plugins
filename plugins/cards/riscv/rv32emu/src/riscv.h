...
#define RV_INT_MEI_SHIFT 11
#define RV_INT_MEI (1U << RV_INT_MEI_SHIFT)
...
/* Create a bare-metal emulator with host-supplied memory and system hooks.
 * Unlike rv_create(), this does not allocate guest RAM, load an ELF image, or
 * install a userspace syscall environment.  It is intended for embedders that
 * own the machine around the hart. */
riscv_t *rv_create_bare(riscv_user_t user, const riscv_io_t *io, riscv_word_t pc);

/* delete a RISC-V emulator */
void rv_delete(riscv_t *rv);

/* Reset a bare-metal emulator created by rv_create_bare(). */
#if RV32_HAS(JIT)
bool rv_step_bare_jit(riscv_t *, uint32_t budget);
void rv_bare_jit_destroy(riscv_t *);
uint64_t rv_bare_jit_instructions(const riscv_t *);
...
/* Step exactly one instruction while caching its decoded representation.
 * Instruction fetches are never cached: the current opcode is fetched and
 * included in the cache key so bus-visible fetches and modified code retain
 * the same semantics as rv_step_debug(). */
void rv_step_cached_debug(void *arg);
...
/* Bare-metal embeddings can present one level-sensitive machine external
 * interrupt.  The source controls mip.MEIP through set(); service() takes the
 * interrupt only when mtvec is configured and the guest has enabled both
 * mstatus.MIE and mie.MEIE.  This deliberately does not instantiate rv32emu's
 * Linux-oriented PLIC/system-MMIO model. */
bool rv_set_machine_external_interrupt(riscv_t *rv, bool asserted);
bool rv_service_machine_interrupts(riscv_t *rv);

#if RV32_HAS(EXT_F)
/* Access the raw IEEE-754 single-precision register representation. */
