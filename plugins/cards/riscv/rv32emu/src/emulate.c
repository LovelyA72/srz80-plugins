...
retranslate:
    ...
    /* Indirect branches consult a branch history table even when stepping a
     * single instruction.  Block translation allocates one per IR, but the
     * stack-decoded IR used here does not, so provide a fresh local table. */
    if (ir.opcode == rv_insn_jalr
        ...
        memset(bht.PC, 0xff, sizeof(bht.PC));
        ir.branch_table = &bht;
    }
...
void rv_step_cached_debug(void *arg)
{
    ...
    if (unlikely(!rv->step_cache)) {
        rv->step_cache = calloc(STEP_CACHE_SIZE, sizeof(*rv->step_cache));
        if (!rv->step_cache) {
            rv_step_debug(rv);
            return;
        }
...
#if !RV32_HAS(SYSTEM)
    /* Keep the reference stepper's userspace exit-address behavior. */
    if (unlikely(rv->PC == PRIV(rv)->exit_addr))
        PRIV(rv)->on_exit = true;
#endif
    ...
    uint32_t insn;
retranslate:
    /* Fetch on every step.  Besides preserving bus semantics, using the
     * fetched word as part of the tag makes modified code self-invalidating. */
    ...
    step_cache_entry_t *entry =
        &rv->step_cache[(pc >> 2) & (STEP_CACHE_SIZE - 1)];
    if (!entry->valid || entry->pc != pc || entry->insn != insn) {
        rv_insn_t ir;
        ...
        /* Publish valid last so a partially initialized entry is never used. */
        entry->valid = false;
        entry->pc = pc;
        entry->insn = insn;
        entry->ir = ir;
    ...
    /* The interpreter's jalr helper requires a BHT even for single stepping.
     * A fresh local table avoids retaining pointers into the separate block
     * cache and keeps cache invalidation straightforward. */
    branch_history_table_t bht;
    bool is_indirect_branch = entry->ir.opcode == rv_insn_jalr;
#if RV32_HAS(EXT_C)
    is_indirect_branch = is_indirect_branch ||
                         entry->ir.opcode == rv_insn_cjalr ||
                         entry->ir.opcode == rv_insn_cjr;
#endif
    if (is_indirect_branch) {
        memset(&bht, 0, sizeof(bht));
        memset(bht.PC, 0xff, sizeof(bht.PC));
        entry->ir.branch_table = &bht;
    }

    entry->ir.impl(rv, &entry->ir, rv->csr_cycle, pc);
}
