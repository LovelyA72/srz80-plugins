...
bool rv_set_machine_external_interrupt(riscv_t *rv, bool asserted)
{
    if (!rv || !rv->bare_mode)
        return false;
    if (asserted)
        rv->csr_mip |= RV_INT_MEI;
    else
        rv->csr_mip &= ~RV_INT_MEI;
    return true;
}

bool rv_service_machine_interrupts(riscv_t *rv)
{
    if (!rv || !rv->bare_mode)
        return false;

    /* In the SRZ80 bare-metal embedding the hart starts and remains in
     * M-mode. Keep the architectural lower-privilege rule as well so this API
     * remains correct if a future embedding adds a controlled mode transition. */
    const bool globally_enabled = rv->priv_mode < RV_PRIV_M_MODE ||
                                  (rv->csr_mstatus & MSTATUS_MIE) != 0;
    if (!globally_enabled || !(rv->csr_mie & RV_INT_MEI) || !(rv->csr_mip & RV_INT_MEI)) {
        return false;
    }

    const uint32_t base = rv->csr_mtvec & ~0x3U;
    const uint32_t mode = rv->csr_mtvec & 0x3U;
    /* A level can be asserted before firmware installs a handler. Preserve it
     * in mip, but never mutate architectural state or call the host trap hook
     * until the guest has elected a valid vector. */
    if (!base)
        return false;

    rv->csr_mepc = rv->PC;
    rv->csr_mcause = UINT32_C(0x80000000) | RV_INT_MEI_SHIFT;
    rv->csr_mtval = 0;
    if (rv->csr_mstatus & MSTATUS_MIE)
        rv->csr_mstatus |= MSTATUS_MPIE;
    else
        rv->csr_mstatus &= ~MSTATUS_MPIE;
    rv->csr_mstatus &= ~(MSTATUS_MIE | MSTATUS_MPP);
    rv->csr_mstatus |= rv->priv_mode << MSTATUS_MPP_SHIFT;
    rv->priv_mode = RV_PRIV_M_MODE;
    rv->PC = mode == 1 ? base + 4U * RV_INT_MEI_SHIFT : base;
    return true;
}

#if RV32_HAS(EXT_F)
void rv_set_freg(riscv_t *rv, uint32_t reg, riscv_word_t in)
{
    assert(rv);
    if (reg < N_RV_REGS)
        rv->F[reg].v = in;
...
riscv_word_t rv_get_freg(riscv_t *rv, uint32_t reg)
{
    assert(rv);
    return reg < N_RV_REGS ? rv->F[reg].v : ~0U;
}

void rv_set_fcsr(riscv_t *rv, riscv_word_t in)
{
    assert(rv);
    rv->csr_fcsr = in & 0xff;
}

riscv_word_t rv_get_fcsr(riscv_t *rv)
{
    assert(rv);
    return rv->csr_fcsr;
}
...
/* Reset just the architectural state needed by the bare-metal embedding API.
 * Keep this separate from rv_reset_hart(): that helper also establishes the
 * userspace ELF ABI in rv32emu-owned RAM. */
static void rv_reset_bare_hart(riscv_t *rv, riscv_word_t pc)
{
...
riscv_t *rv_create_bare(riscv_user_t user, const riscv_io_t *io, riscv_word_t pc)
{
    if (!io || !io->mem_ifetch || !io->mem_read_w || !io->mem_read_s || !io->mem_read_b ||
        !io->mem_write_w || !io->mem_write_s || !io->mem_write_b)
        return NULL;
    ...
    /* rv_step_debug retains a harmless userspace exit-address check. */
    attr->exit_addr = UINT32_MAX;
    rv->data = attr;
    rv->bare_user = user;
    rv->bare_mode = true;
    memcpy(&rv->io, io, sizeof(*io));
    rv_reset_bare_hart(rv, pc);

    rv->block_mp = mpool_create(sizeof(block_t) << BLOCK_MAP_CAPACITY_BITS, sizeof(block_t));
    rv->block_ir_mp =
        mpool_create(sizeof(rv_insn_t) << BLOCK_IR_MAP_CAPACITY_BITS, sizeof(rv_insn_t));
    rv->fuse_mp = mpool_create(FUSE_SLOT_SIZE << BLOCK_IR_MAP_CAPACITY_BITS, FUSE_SLOT_SIZE);
...
{
    return rv && rv->bare_mode ? rv->bare_user : NULL;
}
...
#if RV32_HAS(JIT)
    if (!rv_init_jit(rv)) {
        rv_log_fatal("Init JIT state at warm reboot failed");
        exit(EXIT_FAILURE);
    }
#endif
}
...
#define MAX_OPTS 2
             ...
             */
            char *vblk_device_str = strdup(attr->data.system.vblk_device[i]);
    ...
    if (attr->data.system.vrng_enabled) { /* check for reboot */
        attr->vrng = vrng_new();
        assert(attr->vrng);
