...
#define STEP_CACHE_BITS 12
#define STEP_CACHE_SIZE (1U << STEP_CACHE_BITS)
...
#define WASM_BLOCK_HARD_LIMIT 10000
     ...
     */
#if defined(_MSC_VER)
    __ALIGNED(CACHE_LINE_SIZE) block_l1_cache_t block_l1;
#else
    ...
    /* Optional embedder context used by rv_create_bare(). */
    riscv_user_t bare_user;
    bool bare_mode;
    void *bare_jit; /* Private bounded JIT embedding state; initialized lazily. */

    /* Lazily allocated by rv_step_cached_debug().  Keeping this separate from
     * the block cache makes the one-step API independent of block execution. */
    step_cache_entry_t *step_cache;
 ...
 */
#if !RV32_HAS(SYSTEM) && !RV32_HAS(HOST_IO)
...
#endif /* !RV32_HAS(SYSTEM) && !RV32_HAS(HOST_IO) */
