...
{
    if (!key) { bool added = !set->has_zero; set->has_zero = true; return added; }
    const rv_hash_key_t index = set_hash(key);

    uint8_t count = 0;
    for (; count < SET_SLOTS_SIZE && set->table[index][count]; count++) {
        assert(count < SET_SLOTS_SIZE);
        if (set->table[index][count] == key)
            return false;
    }

    if (count == SET_SLOTS_SIZE) return false;
...
{
    if (!key) return set->has_zero;
    const rv_hash_key_t index = set_hash(key);

    for (uint8_t count = 0; count < SET_SLOTS_SIZE && set->table[index][count]; count++) {
        assert(count < SET_SLOTS_SIZE);
        if (set->table[index][count] == key)
            return true;
    }
