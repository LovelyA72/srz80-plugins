...
/* x86 variable shifts always consume CL. On Win64 RCX owns the VM
 * context while the register allocator's scratch register is R11. */
static inline void emit_shift_reg(struct jit_state *state, int operation, int dst)
{
#if defined(__x86_64__) && defined(_WIN32)
    assert(dst != RCX);
    emit_push(state, RCX);
    emit_mov(state, temp_reg, RCX);
#endif
    emit_alu32(state, 0xd3, operation, dst);
#if defined(__x86_64__) && defined(_WIN32)
    emit_pop(state, RCX);
#endif
}
...
/* Normalize an unsigned RV32 operand before a 64-bit high product. */
static inline void emit_zxtw(struct jit_state *state, int reg)
{
#if defined(__x86_64__)
    emit_alu32(state, 0x89, reg, reg);
#elif defined(__aarch64__)
    emit_logical_register(state, false, LOG_ORR, reg, RZ, reg);
#endif
}
                      ...
                      bool sign)
{
    ...
    if (div || mod) {
        // Use a 64-bit dividend for RV32 signed division. INT32_MIN / -1
        // then fits in the host quotient; storing its low word implements
        // RISC-V overflow semantics without a host divide exception.
        if (sign) { emit_sxtw(state, RAX); emit_sxtw(state, RCX); }
        else { emit_zxtw(state, RAX); emit_zxtw(state, RCX); }
        emit_cmp_imm32(state, RCX, 0);
        uint32_t jump_loc_0 = state->offset;
        emit_jcc_offset(state, JCC_JNE);
        if (mod) emit_mov(state, RAX, RDX);
        else emit_load_imm(state, RAX, UINT32_MAX);
        uint32_t jump_loc_1 = state->offset;
        emit_jcc_offset(state, JCC_JMP);
        emit_jump_target_offset(state, JUMP_LOC_0, state->offset);
        if (sign) { emit1(state, 0x48); emit1(state, 0x99); } // CQO
        else emit_alu32(state, 0x31, RDX, RDX);
        emit_rex(state, 1, 0, 0, 0);
        emit_alu32(state, 0xf7, sign ? 7 : 6, RCX); // IDIVQ / DIVQ
        emit_jump_target_offset(state, jump_loc_1 + 1, state->offset);
    } else {
        if (is64) emit_rex(state, 1, 0, 0, 0);
        emit_alu32(state, 0xf7, 4, RCX);
    }
...
static void prepare_translate(struct jit_state *state)
{
    ...
    emit1(state, 0xff);
    emit1(state, 0xe0 | parameter_reg[1]);
...
extern void rv_bare_jit_memory(riscv_t *, const rv_insn_t *);

static bool emit_bare_memory(struct jit_state *state, riscv_t *rv, rv_insn_t *ir)
{
    if (!rv->bare_mode) return false;
    switch (ir->opcode) {
    case rv_insn_lb: case rv_insn_lbu: case rv_insn_lh: case rv_insn_lhu:
    case rv_insn_lw: case rv_insn_sb: case rv_insn_sh: case rv_insn_sw: break;
    default: return false;
    }
    store_back(state);
    emit_load_imm_sext(state, parameter_reg[1], (intptr_t)ir);
    emit_call(state, (intptr_t)rv_bare_jit_memory);
    emit_exit(state);
    reset_reg();
    return true;
}

void jit_invalidate(riscv_t *rv)
{
...
static void translate(struct jit_state *state, riscv_t *rv, block_t *block)
{
    ...
    for (idx = 0, ir = block->ir_head; idx < block->n_insn && !should_flush;
        ...
        next = ir->next;
        regs_refresh(idx);
        if (!emit_bare_memory(state, rv, ir))
            ((codegen_block_func_t) dispatch_table[ir->opcode])(state, rv, ir);
    }

    if (rv->bare_mode && !should_flush) {
        // Branch/memory blocks already emit a return. This unreachable tail is
        // harmless for them and closes bounded arithmetic-only blocks.
        store_back(state);
                                    ...
                                    block_t *block)
{
    ...
    if (!set_add(&state->set, RV_HASH_KEY(block))) { should_flush = true; return; }
...
static void jit_free_code(void *buffer, size_t size)
{
#if defined(_WIN32)
    (void)size;
...
struct jit_state *jit_state_init(size_t size)
{
    ...
    state->size = size;
#if defined(_WIN32)
    state->buf = VirtualAlloc(NULL, size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    ...
    if (!state->offset_map) {
        jit_free_code(state->buf, state->size);
        free(state);
        return NULL;
    }
