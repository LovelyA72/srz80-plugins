...
constexpr uint32_t config_properties = 2;
...
struct Card {
    ...
    int32_t priority = 0;
    uint32_t chip_clock_hz = default_clock_hz;
    uint32_t sample_rate = default_clock_hz / 224;
    ...
    Card(std::span<const uint8_t> image, uint32_t clock)
        : chip_clock_hz(clock), sample_rate((clock + 112) / 224), engine(image, clock) {}
...
SrhStatus SRH_CALL info(void *, uint32_t index, SrhProperty *out) {
    ...
    if (index < config_properties) {
        static const char *names[] = {"base", "chip_clock_hz"};
        static const char *descriptions[] = {"First MultiPCM port", "YMW258 input clock"};
...
SrhStatus SRH_CALL save_payload(void *context, uint8_t *buffer, uint64_t *size) {
    ...
    const uint64_t required = 3 + 4 + engine.size();
...
SrhStatus SRH_CALL load_payload(void *context, const uint8_t *buffer, uint64_t size) {
    ...
    if (size != 7 + engine_size || !card.engine.load_state(engine_state)) return SRH_INVALID;
