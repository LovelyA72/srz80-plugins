 ...
 */
/**
 * @brief Zapise znamenkovy displacement jako dekadicke cislo pro Motorola styl.
 *
 * Pomocna funkce pro renderovani indexovane adresace ve stylu sdas/sdcc:
 * format "(d,IX)" / "(-d,IY)". Sdas-z80 prijima hex i dekadicky zapis
 * displacementu, ale dekadicka forma je citelnejsi a pro typicke
 * displacement v rozsahu -128..+127 jednoznacna. Zaporne hodnoty maji
 * leading minus, kladne a nula jsou bez znamenka.
 *
 * Priklady: 5 -> "5", -2 -> "-2", 0 -> "0", -128 -> "-128", 127 -> "127".
 *
 * @param[in,out] ctx  Kontext bufferu. Nesmi byt NULL.
 * @param[in]     disp Znamenkovy displacement (-128 az +127).
 *
 * @pre ctx != NULL
 */
static void fmt_displacement_dec_signed(buf_ctx_t *ctx, s8 disp)
{
    /* Rozsah s8 je -128..+127. snprintf produkuje max 4 znaky ("-128"). */
    char tmp[8];
                        ...
                        const z80_dasm_format_t *fmt)
{
    ...
    case Z80_OP_MEM_IX_D:
        ...
        if (fmt->ix_iy_style == Z80_IX_MOTOROLA) {
             ...
             * prvni, oddeleny carkou. Sdas-z80 ho takto vyzaduje. */
