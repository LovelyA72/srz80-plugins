...
class x1_010_core : public vgsound_emu_core
{
	...
	private:
		...
		class voice_t : public vgsound_emu_core
		{
			private:
				class flag_t : public vgsound_emu_core
				{
					public:
						...
						inline u8 pack() const
						{
							return (m_div << 7) | (m_env_oneshot << 2) | (m_wavetable << 1) | m_keyon;
						}

						inline void unpack(u8 data) { write(data); }
			...
			public:
				...
				// State persistence helpers. The format is explicit little-endian
				// bytes so it does not depend on host structure packing.
				static constexpr std::size_t serialized_size = 0x19;
				void save_state(u8 *&out) const;
				bool load_state(const u8 *&in, const u8 *end);
	...
	public:
		...
		static constexpr std::size_t serialized_size = 0x2000 + 16 * voice_t::serialized_size;
		void save_state(u8 *buffer, std::size_t size) const;
		bool load_state(const u8 *buffer, std::size_t size);
