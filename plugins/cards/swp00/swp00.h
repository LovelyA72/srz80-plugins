...
using s16 = int16_t; using s32 = int32_t; using s64 = int64_t; using offs_t = uint32_t;

class Engine {
public:
    static constexpr uint32_t address_space_size = 1u << 24;
    static constexpr uint32_t voice_count = 32;
    explicit Engine(std::span<const uint8_t> rom);
    void reset();
    uint8_t read(uint32_t address, bool peek = false);
    void write(uint32_t address, uint8_t data);
    std::array<int16_t, 2> generate();
    std::vector<uint8_t> save_state() const;
    bool load_state(std::span<const uint8_t> bytes);
    uint32_t inspect(uint32_t voice, uint32_t field) const;
    static constexpr std::array<const char *, 22> fields = {
        "Address", "Start", "Loop", "Pitch", "Format", "Pan", "Global",
        "Dry", "Reverb", "Chorus", "Variation", "LFORate", "LFOPitch", "LFOAmplitude",
        "Attack", "AttackLevel", "Decay", "DecayLevel", "Envelope", "EnvelopeStage", "Playing", "Output"
    };
private:
    struct wave_access_cache {
        std::vector<uint8_t> rom;
        uint8_t read_byte(uint32_t a) const {
            a &= 0xffffff;
            return a < rom.size() ? rom[a] : 0xff;
        }
        uint16_t read_word(uint32_t a) const { return read_byte(a) | (uint16_t(read_byte(a + 1)) << 8); }
    } m_rom_cache;
    template<class Archive> void archive(Archive &ar);
	...
	template<size_t Size> struct delay_block {
		...
		delay_block(Engine *swp, std::array<s32, Size> &buffer);
