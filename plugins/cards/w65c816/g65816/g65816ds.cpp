...
u32 g65816_disassembler::instruction_cycles(offs_t pc, const data_buffer &opcodes) const
{
	// These are the CLK_* values used by the imported execution core. Keep
	// this calculation here, next to the opcode/addressing-mode table, rather
	// than making the card guess timing from formatted assembly text.
	constexpr u32 op = 1, read8 = 1, read16 = 2, write8 = 1, write16 = 2;
	constexpr u32 rmw8 = 3, rmw16 = 5;
	constexpr u32 implied = 1, relative8 = 1, relative16 = 2;

	const opcode_struct &opcode = s_opcodes[opcodes.r8(pc & 0xffffff)];
	const bool m8 = m_config->get_m_flag();
	const bool x8 = m_config->get_x_flag();
	const u32 accumulator_width = m8 ? read8 : read16;
	const u32 index_width = x8 ? read8 : read16;

	const auto address_cycles = [=](unsigned mode) -> u32 {
		switch (mode) {
		case IMP: case ACC: case IMM: return 0;
		case RELB: return relative8;
		case RELW: return relative16;
		case A: return 2;
		case AL: case ALX: return 3;
		case AX: case AY: case DX: case DY: return 2;
		case AI: case AXI: return 4;
		case D: return 1;
		case DI: case DIY: return 3;
		case DLI: case DLIY: return 4;
		case DXI: return 4;
		case S: return 2;
		case SIY: return 5;
		default: return 0;
	...
	const auto write_address_cycles = [](unsigned mode) -> u32 {
		switch (mode) {
		case IMP: case ACC: case IMM: return 0;
		case A: return 2;
		case AL: case ALX: return 3;
		case AX: case AY: return 3;
		case AI: case AXI: return 4;
		case D: return 1;
		case DI: case DIY: return 3;
		case DLI: case DLIY: return 4;
		case DX: case DY: return 2;
		case DXI: return 4;
		case S: return 2;
		case SIY: return 5;
		default: return 0;
	...
	const auto read_operand = [&](u32 width) {
		return op + width + address_cycles(opcode.ea);
	};
	const auto modify_operand = [&](u32 width) {
		return op + (width == read8 ? rmw8 : rmw16) + write_address_cycles(opcode.ea);
	};
	const auto store_operand = [&](u32 width) {
		return op + width + write_address_cycles(opcode.ea);
	};

	switch (opcode.m_name) {
	case op::ADC: case op::AND: case op::BIT: case op::CMP:
	case op::EOR: case op::LDA: case op::ORA: case op::SBC:
		return read_operand(opcode.flag == M ? accumulator_width : index_width);
	case op::CPX: case op::CPY: case op::LDX: case op::LDY:
		return read_operand(index_width);
	case op::ASL: case op::DEC: case op::INC: case op::LSR:
	case op::ROL: case op::ROR: case op::TRB: case op::TSB:
		if (opcode.ea == ACC || opcode.ea == IMP)
			return op + implied;
		return modify_operand(accumulator_width);
	case op::STA: case op::STZ:
		return store_operand(accumulator_width);
	case op::STX: case op::STY:
		return store_operand(index_width);

	case op::BRK: case op::COP:
		return m_config->get_e_flag() ? 7 : 8;
	case op::BRA: return op + implied + relative8;
	case op::BRL: return op + implied + relative16;
	case op::BCC: case op::BCS: case op::BEQ: case op::BMI:
	case op::BNE: case op::BPL: case op::BVC: case op::BVS:
		return op + relative8;
	case op::JML: return op + 4 + 1;
	case op::JMP:
		return opcode.ea == AI || opcode.ea == AXI ? op + 4 : op + address_cycles(opcode.ea);
	case op::JSL: return 8;
	case op::JSR: return opcode.ea == AXI ? 8 : 6;
	case op::MVN: case op::MVP: return 7;
	case op::PEA: return 5;
	case op::PEI: case op::PER: return 6;

	case op::PHA: return op + (m8 ? write8 : write16) + 1;
	case op::PHX: case op::PHY: return op + (x8 ? write8 : write16) + 1;
	case op::PHB: case op::PHK: case op::PHP: return op + write8 + 1;
	case op::PHD: return op + write16 + 1;
	case op::PLA: return op + accumulator_width + 2;
	case op::PLX: case op::PLY: return op + index_width + 2;
	case op::PLB: return op + read8 + 2;
	case op::PLD: return op + read16 + 2;
	case op::PLP: return op + read8 + 2;
	case op::RTI: return m_config->get_e_flag() ? 7 : 8;
	case op::RTL: case op::RTS: return 6;
	case op::REP: case op::SEP: return op + read8 + 1;
	case op::XBA: return op + implied + 1;
	case op::STP: case op::WAI: return 1;
	default:
		return op + implied;
	}
}
